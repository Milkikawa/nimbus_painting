...
上述控制参数会按对应语法从提示词中清理，避免常规的单个控制 token 出现在最终传递给上游的 prompt 中；同类参数由第一个匹配项决定取值。每类控制参数不应重复填写；重复且无空格紧邻的 token（例如 `--seed 1,--seed 2,prompt`）受分隔符匹配方式影响，不保证全部清理。可选编号范围、模型类型、可用状态及特殊参数规则可在后台「模型目录」中查看和维护；默认值及参数边界在「基础设置」中查看。

### 处理路径

1. **提取用户文本** — 从 Chat Completions 的 `messages` 末尾向前查找，使用最后一条非空且 `role` 为 `user` 的文本。`content` 为字符串时直接去除首尾空白；为数组时只提取其中 `type` 为 `text` 且 `text` 为字符串的项目，以空格连接后再去除首尾空白。
2. **规范化文本** — 将换行替换为空格，并将常见中文标点规范为对应的英文标点，便于后续统一解析。
3. **解析并移除控制参数** — 识别模型编号、尺寸、seed、steps 与 cfg；同类参数由第一个匹配项决定取值，并按对应语法执行控制 token 清理。每类参数不应重复填写；重复且无空格紧邻的 token 不保证全部清理。随后清理逗号、分号产生的空项，形成“清理后用户 prompt”。
4. **执行模型目录准入与特殊规则** — 选中的模型必须存在、启用且可用于图片生成，否则请求返回 400。模型目录还可以强制覆盖 steps，或禁止追加后台正面提示词组。
5. **组合正负提示词** — 在模型允许时，将后台当前选中的正面组追加到清理后用户 prompt；`negative_prompt` 则独立取自后台当前选中的负面组。
6. **构造上游请求** — 最终发送的 payload 包含 `prompt`、`negative_prompt`、`width`、`height`、`steps`、`cfg`、`model_index` 和 `seed`。若清理后用户 prompt 为空，请求直接返回 400，不会调用上游。

### 参数来源与优先级

| 项目              | 来源与优先级（从高到低）                                          | 规则                                                                                                                                                                                                                            |
| ----------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 模型              | 用户提示词中第一个 `sdN` → 经目录校验、必要时调整过的后台默认模型 | 第一个匹配项决定取值，并按对应语法清理 `sdN`；重复 token 的清理受上述限制。用户明确指定的模型若不存在、未启用或不可用于生图，会直接返回 400，不会静默回退；仅后台默认模型自身失效时，加载设置阶段才会改用目录中的可用图片模型。 |
| 尺寸              | 第一个尺寸 token 的合法宽高 → 后台默认宽高                        | 支持 `宽x高` 或 `宽*高`。只考虑第一个匹配项，且宽、高均在后台最小/最大尺寸范围内时才覆盖默认值；无效时保留默认宽高。单个已匹配但越界的尺寸 token 仍会按语法清理；重复 token 的清理受上述限制。                                  |
| seed              | 第一个 `--seed` 的合法值 → 随机值                                 | 只考虑第一个匹配项，合法范围为 `0..2147483646`。未提供时生成同范围内的随机值；单个已匹配但越界的值仍会按语法清理，并保留本次生成的随机值。重复 token 的清理受上述限制。                                                         |
| steps             | 模型规则 `force_steps` → 第一个 `--steps` 的合法值 → 后台默认值   | 只考虑第一个匹配项，用户值合法范围为 `1..50`；模型强制值可覆盖用户值和后台默认值。单个已匹配但越界的用户值仍会按语法清理，并回退到后台默认值，之后仍可能被模型规则覆盖；重复 token 的清理受上述限制。                           |
| cfg               | 第一个 `--cfg` 的合法值 → 后台默认值                              | 只考虑第一个匹配项，合法范围为 `1..10`。单个已匹配但越界的值仍会按语法清理，并保留后台默认值；重复 token 的清理受上述限制。                                                                                                     |
| 正向 `prompt`     | 清理后用户 prompt + 后台当前选中的正面组                          | 正面组非空且模型允许追加时才会附加；模型规则关闭默认正面提示词追加后，最终正向 prompt 仅使用清理后用户 prompt。                                                                                                                 |
| `negative_prompt` | 后台当前选中的负面组                                              | 没有用户负面提示词控制语法；未选中负面组时为空。                                                                                                                                                                                |

这里涉及四种容易混淆的提示词状态：

- **原始提示词**：从最后一条非空 `user` 消息提取出的文本，尚未规范化或移除控制参数。
- **清理后用户 prompt**：完成标点与换行规范化、控制参数移除以及空项清理后的用户文本；它只是处理中间结果，请求日志没有单独字段记录这一状态。
- **最终正面 prompt**：清理后用户 prompt 按模型规则决定是否追加后台正面组后的结果，即上游 payload 中的 `prompt`。
- **`negative_prompt`**：后台当前选中的负面组内容，与正面 prompt 分开传递，不从用户文本中解析。

请求日志中的 `RawPrompt` 记录原始提示词，`FinalPrompt` 记录最终正面 prompt，`NegativePrompt` 记录传给上游的负面提示词；日志不会把“清理后用户 prompt”作为独立字段保存。

> 对尺寸、seed、steps、cfg 而言，单个已匹配但数值越界的控制 token 仍会按对应语法从 prompt 中清理，对应参数则保留后台默认值或本次随机 seed；重复 token 的清理仍受上述限制。模型准入不同：用户明确指定不可用的 `sdN` 时会直接拒绝请求。

---

## 模型目录

后台「模型目录」维护的是本地上游模型能力配置，用于未来请求的准入和兼容处理。`/v1/models` 只返回下游代理模型入口（`sd-generate`、`sd-edit`）；完整上游目录由后台「模型目录」管理和查看：
...
生成成功后，项目会下载上游图片并保存至本地。图片数据会先流式写入最终目录中的隐藏临时文件，完整写入并成功关闭后再通过同目录 `rename` 发布为最终文件，避免下载中断时在正式路径留下可见的半文件。该流程不对每张图片执行 `fsync`，也不会进行第二次完整文件拷贝，以减少机械硬盘上的额外 I/O。
...
日志详情会单独读取仍有效的本地归档 URL，并且只自动加载本地归档图片；上游 `image_url` 不会作为缩略图、大图或加载失败后的备用图片自动请求。需要查看上游原图时，可由用户显式点击「上游原图」链接。若本地归档缺失、已删除或加载失败，页面显示「本地归档不可用」占位。

成功加载的本地缩略图可以在页面内打开大图预览。大图支持关闭按钮、点击遮罩、`Esc` 与键盘焦点管理；移动端请求详情使用全屏抽屉展示。页面切换、登录状态失效或退出后台时，会中止仍在进行的日志请求并清理抽屉、大图和全局事件监听。
