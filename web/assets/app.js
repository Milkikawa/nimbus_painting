...
el.password.addEventListener("keydown", (event) => {
  if (event.key === "Enter") state.status?.initialized ? login() : initAdmin();
});
document
...
async function refreshStatus() {
  ...
  el.serviceStatus.textContent = state.status.upstream_configured
    ? "上游已配置"
    : "待配置上游";
  ...
  if (el.sidebarStatusDot) {
    el.sidebarStatusDot.className =
      "status-dot " + (state.status.upstream_configured ? "online" : "offline");
    el.sidebarStatusText.textContent = state.status.upstream_configured
      ? "服务运行中"
      : "待配置上游";
  }
}
